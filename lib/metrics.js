 ...
 *
 * APPROACH: Instead of limiting metrics to a predefined focal brand list,
 * this module discovers all unique brands across all replicates and
 ...
 * @param {Object[]} focalBrands  – array of { brand, visibility_group, sub_category }
 *                                  used only to tag visibility_group; metrics are
 *                                  computed for every brand regardless
 ...
 */
function calculateMetrics(rawResults, focalBrands, subCategory, modelId, promptCondition = '') {
  const R = rawResults.length;  // total number of replicates
  if (R === 0) return [];

  // ── Step 1: Build a focal-brand lookup for visibility tagging ────
  const focalLookup = {};
  for (const fb of focalBrands) {
    focalLookup[fb.brand] = fb.visibility_group;
  }

  // ── Step 2: Discover all unique brands across all replicates ─────
  ...
  // ── Step 3: For each brand, scan all replicates for rank ──────────
  ...
  // ── Step 4: Build output rows sorted by total mentions desc ──────
  const metrics = Object.entries(brandData)
    .map(([brand, data]) => ({
      ...
      brand,
      visibility_group: focalLookup[brand] || 'other',
      n_replicates: R,
