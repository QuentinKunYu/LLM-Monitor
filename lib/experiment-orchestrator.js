...
function buildProtocol({
  ...
  const includeContextFree = input.includeContextFree !== false;
  const includeNeedsBased = input.includeNeedsBased !== false;
  if (!includeContextFree && !includeNeedsBased) {
    throw new Error('Select at least one condition to run.');
  }
  ...
  if (includeContextFree && (!promptTemplate || !/\[category\]/i.test(promptTemplate))) {
    throw new Error('The context-free prompt must include a [category] placeholder.');
  }

  const selectedNeedsPrompts = includeNeedsBased
    ? needsPrompts.filter(prompt => categoryNames.includes(prompt.sub_category))
    : [];
  if (includeNeedsBased && !selectedNeedsPrompts.length) {
    throw new Error('No needs-based prompts match the selected categories.');
  }

  const recommendationTasks = [];
  if (includeContextFree) {
    for (const subCategory of categoryNames) {
...
function calculateExperimentMetrics(results, expectedTasks) {
  ...
  for (const [key, rows] of groupedRows.entries()) {
    ...
    for (const metric of calculateMetrics(rows, subCategory, modelId, promptCondition)) {
      metrics.push({
        ...metric,
...
function calculateExperimentThemeMetrics(results, expectedTasks) {
  return calculateExperimentBreakdown({
    results,
    expectedTasks,
    dimensionName: 'theme',
    dimensionFor: row => String(row.theme || '').trim(),
    extraContext: rows => ({
      prompt_id_count: new Set(rows.map(row => row.prompt_id).filter(Boolean)).size,
    }),
  });
}

function calculateExperimentBreakdown({
  results,
  expectedTasks,
  dimensionName,
  dimensionFor,
  ...
  const eligibleTasks = expectedTasks.filter(task => dimensionFor(task));
  const keyFor = row => [
    row.sub_category,
    row.model_id,
  ...
  const groupedRows = groupBy(successful, keyFor);
  const expectedGroups = groupBy(eligibleTasks, keyFor);
  const output = [];

  for (const [key, rows] of groupedRows.entries()) {
    const [subCategory, modelId, promptCondition, dimensionValue] = key.split('|');
    const expected = expectedGroups.get(key)?.length || rows.length;
    for (const metric of calculateMetrics(rows, subCategory, modelId, promptCondition)) {
      output.push({
        ...metric,
        [dimensionName]: dimensionValue,
        ...(extraContext ? extraContext(rows) : {}),
        denominator: rows.length,
        expected_observations: expected,
        provisional: rows.length !== expected,
      });
  ...
  return output.sort((a, b) => (
    ...
    a.model_id.localeCompare(b.model_id) ||
    a.prompt_condition.localeCompare(b.prompt_condition) ||
    String(a[dimensionName]).localeCompare(String(b[dimensionName])) ||
    Number(b['BRP@5']) - Number(a['BRP@5'])
  ));
}
