...
// ── OpenAI ──────────────────────────────────────────────────────────
async function callOpenAI(prompt, modelName, temperature, maxTokens, webSearch, apiKey) {
  ...
  const temperatureParam = Number.isFinite(temperature) && !finalModelName.startsWith('gpt-5.5')
    ? { temperature }
    : {};
...
// ── Anthropic ───────────────────────────────────────────────────────
async function callAnthropic(prompt, modelName, temperature, maxTokens, webSearch, apiKey) {
  ...
  if (Number.isFinite(temperature) && !modelName.includes('opus-4-7')) {
...
// ── Google Gemini ───────────────────────────────────────────────────
async function callGoogle(prompt, modelName, temperature, maxTokens, webSearch, apiKey) {
 ...
 */
async function sendPrompt(provider, modelName, prompt, temperature, maxTokens, webSearch = false, apiKey = '') {
  switch (provider.toLowerCase()) {
    case 'openai':
      return callOpenAI(prompt, modelName, temperature, maxTokens, webSearch, apiKey);
    case 'anthropic':
      return callAnthropic(prompt, modelName, temperature, maxTokens, webSearch, apiKey);
    case 'google':
      return callGoogle(prompt, modelName, temperature, maxTokens, webSearch, apiKey);
    default:
