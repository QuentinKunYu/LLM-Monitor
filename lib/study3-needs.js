...
function calculateNeedsMetrics(rawRows) {
  ...
  for (const [key, rows] of groupRows(successfulRows, row => [
    ...
    metrics.push(...calculateMetrics(rows, subCategory, modelId, promptCondition));
  }
  ...
  for (const [key, rows] of groupRows(successfulRows, row => [
    ...
    const themeMetrics = calculateMetrics(rows, subCategory, modelId, promptCondition);
